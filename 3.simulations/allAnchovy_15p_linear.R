...
parametersData = read.csv(file = "/home1/datawork/clujanpa/sensitivity/paper_protocol/b3_15p_linear/data_perturbation/osm_parameters_allAnchovy-protocol.csv",
                          header = TRUE, sep = ",", stringsAsFactors = FALSE)
parametersData$scale = rep("linear", dim(parametersData)[1])

# Estimation of min and max (limits of parameter distribution)
parametersData$percentage = rep(0.15, dim(parametersData)[1])
...
run_model = function(par, names, ...) {
  ...
  setwd("/home1/datawork/clujanpa/sensitivity/paper_protocol/b3_15p_linear")
...
start = date()
fixed15p_linear_allAnchovy = run_experiments(X = doe, FUN = run_model, names=parametersData$parameter)
end   = date()

setwd("/home1/datawork/clujanpa/sensitivity/paper_protocol/b3_15p_linear")
saveRDS(object = fixed15p_linear_allAnchovy, file = "fixed15p_linear_allAnchovy.rds")
