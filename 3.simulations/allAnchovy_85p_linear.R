...
# directory
setwd("/home2/datahome/ipitavac/sensitivity_osmose")
...
parametersData = read.csv(file = "/home2/datawork/ipitavac/sensitivity/paper_protocol/b3_85p_linear/data_perturbation/osm_parameters_allAnchovy-protocol.csv",
                          header = TRUE, sep = ",", stringsAsFactors = FALSE)
parametersData$scale = rep("linear", dim(parametersData)[1])

# Estimation of min and max (limits of parameter distribution)
parametersData$percentage = rep(0.85, dim(parametersData)[1])
...
run_model = function(par, names, ...) {
  ...
  setwd("/home2/datawork/ipitavac/sensitivity/paper_protocol/b3_85p_linear")
...
start = date()
fixed85p_linear_allAnchovy = run_experiments(X = doe, FUN = run_model, names=parametersData$parameter)
end   = date()

setwd("/home2/datawork/ipitavac/sensitivity/paper_protocol/b3_85p_linear")
saveRDS(object = fixed85p_linear_allAnchovy, file = "fixed85p_linear_allAnchovy.rds")
